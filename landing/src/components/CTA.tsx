...
function CopyLine({ text }: { text: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <button
        onClick={copy}
        className="flex-shrink-0 text-xs text-zinc-500 hover:text-accent transition-colors"
...
export default function CTA() {
  ...
  return (
    <section id="get-started" className="py-24 px-6 bg-surface">
      <div ref={ref} className="max-w-2xl mx-auto text-center">
        {/* Terminal */}
        <div className="rounded-xl border border-border bg-[#0a0a0b] overflow-hidden text-left mb-10">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
            <span className="w-3 h-3 rounded-full bg-[#ff5f57]" />
            <span className="w-3 h-3 rounded-full bg-[#febc2e]" />
            <span className="w-3 h-3 rounded-full bg-[#28c840]" />
          </div>
          <div className="p-5 font-mono text-sm flex flex-col gap-2">
            {commands.map((cmd) => (
